...
```bash
lambda-eservice-bot$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-eservice-bot$ sam local start-api
lambda-eservice-bot$ curl http://localhost:3000/
```
...
```bash
lambda-eservice-bot$ sam logs -n HelloWorldFunction --stack-name lambda-eservice-bot --tail
```
...
```bash
lambda-eservice-bot$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name lambda-eservice-bot
```
